...
from latent_analysis.helpers.yeast_prepare_helper import kinetic_model_prepare_ignore_int_issues
...
class check_jacobian():
    ...
    def _load_ktmodels(self, thermo_file, kinetic_file):
        
        path_to_tmodel = thermo_file
        path_to_kmodel = kinetic_file
        ...
        # self.kmodel.prepare()
        # Prep and compile
        # NOTE THAT FOR SOME REASON YEAST DATA NEEDS A TRICK TO LOAD PROPERLY
        self.kmodel = kinetic_model_prepare_ignore_int_issues(self.kmodel)

        self.kmodel.compile_jacobian(sim_type = QSSA, ncpu = self.NCPU)

    def _load_ssprofile(self, ss_file, ss_idx):
